...
Graph* Complement(Graph *g) {
	Graph *g_c = initGraph(1, 0);	//补图为无向图，但由于输出html文件仅支持directional=1，故将所有边的方向视为从下标小至下标大
	int h, t;

	for (int i = 0; i < g->nv; i++) {		//补图顶点与原图一致
		addVex(g_c, g->v[i].id, g->v[i].w);
	}
	for (int i = 0; i < g->nv; i++) {		//先在补图中连所有边，成为完全图
		for (int j = i + 1 ; j < g->nv; j++) {
			addEdge(g_c, i * g->nv + j, g->v[i].id, g->v[j].id, 1);
		}
	}

	for(int i = 0; i < g->ne; i++) {		//再删去原图中有的边
		getVexByEdge(g, g->e[i].id, &h, &t);
		deleteEdge(g_c, h<t ? h*g->nv+t : t*g->nv+h);	//g_c中id的计算是由连边成完全图时的id公式推知
	}
	return g_c;
}
...
void BFS(Graph* g, int id) {
	bool visited[MAXVERTEX];		//标记是否搜索过
	int vex, next, h, t, fringe[MAXVERTEX];
	int *left = fringe, *right = fringe;		//队列指针
	for (int i = 0; i < g->nv; i++) {		//初始化
		visited[i] = false;
	}
	...
	*right = vex;
	...
	while(left < right) {		//队列中不再有需要搜索的点时结束
		vex = *left;
		left++;			//出队并输出之
		printf(" %d -> ", g->v[vex].id);

		for(int i = 0; i < g->v[vex].degree; i++) {
			getVexByEdge(g, g->v[vex].edges[i], &h, &t);
			next = getVexIdx(g, h != vex ? h : t);		//将其所有未搜索过的相邻顶点入队
			if (!visited[next]) {
				*right = next;
				right++;
				visited[next] = true;
...
void DFS(Graph* g, int id) {
	...
	int vex, next, h, t, fringe[MAXVERTEX];
	int *top = fringe;			//相比于 BFS, DFS 改为使用栈，其余不变
	for (int i = 0; i < g->nv; i++) {
		visited[i] = false;
	}
...
int ConnectedComponents(Graph* g) {
	bool visited[MAXVERTEX];
	int vex, next, h, t, cnt = 0;		//用cnt计数
	int fringe[MAXVERTEX], *top = fringe;
	for (int i = 0; i < g->nv; i++) {
		visited[i] = false;
	}
	...
	for (int j = 0; j < g->nv; j++) {
		if (!visited[j]) {
			...
			*top = vex;
			...
			while(top > fringe) {	//基于DFS实现每个连通片的搜索
				top--;
				vex = *top;
				printf(" %d -> ", g->v[vex].id);
...
int main() {
		...
		printgraph(g);
		saveGraph(g, "Dijkstra.html");
		Dijkstra(g, 3, 4);

		printf("========== Complement ==========\n");
		g = Complement(g);
		printgraph(g);
		saveGraph(g, "Complement.html");
