...
//在原框架基础上，增加bool类型参数first，以重新初始化idx=0
bNode *buildBTreeByPrelist(int *prelist, int len, bool first) {
	bNode *t;
	static int idx; //递归中每次调用自身都需要访问的“特殊”变量用静态变量或全局变量
	if (first == true) {
		idx = 0;
	}
	...
	t->lchild = buildBTreeByPrelist(prelist, len, false);
	t->rchild = buildBTreeByPrelist(prelist, len, false);
	return t;
...
bNode *buildBTreeByPI(int *pre, int pl, int pu, int *in, int il, int iu) {
	printf("\n%d-%d,%d-%d", pl, pu, il, iu);
	int il1, iu1, il2, iu2, pl1, pl2, pu1,
	    pu2; //根节点把中序序列分成两个子序列，同样得到先序序列的两个子序列，这是子序列的下标
	if (pu - pl != iu - il)
		exit(0); //检查输入参数
	if (pu - pl < 0)
...
/*** 以下为本次实验所要求实现的操作 ***/
//删除节点value=x 的节点及其子树
void del_subtree(bNode *root, int val) {
	if (!root) {
		return;
	}
	if (root->lchild ? root->lchild->data.value == val : 0) {	//如果有左子树，则考察左子节点的值，如果满足条件则删除之
		root->lchild = NULL;
		return;
	}
	if (root->rchild ? root->rchild->data.value == val : 0) {	//如果有右子树，则考察右子节点的值，如果满足条件则删除之
		root->rchild = NULL;
		return;
	}

	del_subtree(root->lchild, val);		//value可能重复，因此需要遍历到底
	del_subtree(root->rchild, val);
}

//给定id1 和id2，输出其最近共同祖先id，节点的id 具有唯一性
int common_ancestor(bNode *root, int id1, int id2) {
	if (!root) {
		return 0;
	}
	bPrintNode(root);	//此处输出是为清晰展现查找过程，下同
	printf("    search!");
	int flag0 = (root->data.id == id1 || root->data.id == id2) ? 1 : 0;
	int flag1 = common_ancestor(root->lchild, id1, id2);
	int flag2 = common_ancestor(root->rchild, id1, id2);
	if (flag0 + flag1 + flag2 >= 2) {		//flag==2表示该节点与其子树中已经同时包含id1与id2二者
		printf("\noutput = %d ", root->data.id); 	//当前节点为最近的共同祖先，输出其编号
		bPrintNode(root);
		printf("    return 0");		//只输出最近的共同祖先，不再输出非最近的
		return 0;
	} else if (flag0 + flag1 + flag2 == 1) {
		bPrintNode(root);
		printf("    return 1");
		return 1;	//表示在该节点或其子树中恰有id1与id2中的其一
	} else {
		bPrintNode(root);
		printf("    return 0");
		return 0;
...
//给定id 值，输出从根节点到id 值节点的路径，用“左右右...”的左右孩子指针标记从根到节点的路径
int lr_route(bNode *root, int id) {
	if (!root) {
		return 0;
	}
	if (root->data.id == id) {
	...
	int left = lr_route(root->lchild, id);
	int right = lr_route(root->rchild, id);
	if (left != 0) {	//表示在左子树下找到目标节点，则在返回值二进制末位写0
		return 2 * left;
	}
	if (right != 0) {	//表示在右子树下找到目标节点，则在返回值二进制末位写1
		return 2 * right + 1;
	}
	return 0;
}

//输出二叉树节点中最大的value 和最小的value 之差
int diff_value(bNode *root, bool first){		//追加bool类型参数first，以初始化static变量
	if(!root){
		return -1;
	}
	static int max;
	static int min;
	if(first){	//内部递归调用时first恒为false，每次递归仅第一次时执行本if块
		max = root->data.value;
		min = root->data.value;
	}

	if(root->data.value > max){		//更新max与min
		max = root->data.value;
	}
	if(root->data.value < min){
		min = root->data.value;
	}
	diff_value(root->lchild, false);	//递归遍历
	diff_value(root->rchild, false);
	return max-min;
}

int main() {
	int val[] = {41, 32, 17, 7, 13, 22, 16};
	int n = 7;
	...
	printf("\n测试二叉树生成 edge :");
	int edge[7] = {1, 2, -1, 4, 2, 6, 4}; //增加边数组
	...
	t1 = buildBTreeByPrelist(prelist, 15, true);
	preTraverseTree(t1, cPrintNode); //测试先序遍历
	inTraverseTree(t1, cPrintNode); //测试中序遍历
	postTraverseTree(t1, bPrintNode); //测试后序遍历
	...
	//测试本次实验所要求实现的操作
	printf("\n\n__________del_subtree__________");
	t1 = buildBTreeByEdge(val, edge, n);
	del_subtree(t1, 16);
	preTraverseTree(t1, bPrintNode);
	saveTree(t1, "del_subtree.html");	//结果用图输出
	DestroyBTree(t1);

	printf("\n\n__________common_ancestor__________");
	t1 = buildBTreeByEdge(val, edge, n);
	common_ancestor(t1, 5, 3);
	DestroyBTree(t1);

	printf("\n\n__________lr_route__________\n");
	t1 = buildBTreeByEdge(val, edge, n);
	int route = lr_route(t1, 5);	//函数返回值对应一个二进制数，除首位恒为1外，其余每位从低位住高位计，为0表示left，为1表示right
	while (route > 1) {
		if (route % 2 == 0) {	
			printf("-> left ");
		} else {
	...
	printf("\n\n__________diff_value__________\n");
	t1 = buildBTreeByEdge(val, edge, n);
	printf("%d", diff_value(t1, true));
	DestroyBTree(t1);
}
